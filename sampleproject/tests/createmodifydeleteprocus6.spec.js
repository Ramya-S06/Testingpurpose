...
test('Customer + Project Flow (Create, Modify, Delete)', async ({ page }) => {
    ...
    const updatedProject = projectName + '_Updated';

    await page.fill('[name="projectName"]', updatedProject);
    await page.click('text=Update');

    await page.waitForTimeout(2000);

    // 7️⃣ Delete Project
    await page.locator(`text=${updatedProject}`).first().click();
